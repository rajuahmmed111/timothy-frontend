...
export default function PaymentPage() {
  ...
  const [createHotelBooking, { isLoading: isCreating }] =
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (bookingData?.alreadyCreated) {
        const confirmed = {
          ...bookingData,
          bookingId:
            bookingData?.existingBookingId ||
            "BK" + Math.floor(10000000 + Math.random() * 90000000),
          bookingDate: new Date().toISOString().split("T")[0],
          paymentMethod: `Card ending in ${cardNumber.slice(-4)}`,
        };
        navigate("/booking-confirmation", { state: confirmed });
        return;
      }
      const payload = {
        rooms: Number(bookingData?.guests?.rooms || 1),
              ...
              <form onSubmit={handleSubmit} className="p-6">
                <div className="space-y-6">
                      ...
                      <input
                        ...
                        className={`w-full pl-11 pr-4 py-3 border ${
                          errors.cardNumber
                            ? "border-red-500"
                            : "border-gray-300"
                        } rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-colors`}
                      />
                    </div>
                    {errors.cardNumber && (
                  ...
                  <button
                    ...
                    className={`w-full bg-sky-600 hover:bg-sky-700 text-white font-medium py-3 px-6 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 flex items-center justify-center ${
