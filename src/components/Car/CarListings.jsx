...
export default function CarListings() {
  ...
  const carsData = cars.map((car, idx) => ({
    ...car,
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
              <div>
                ...
                {selectedCar.raw?.carType}
              </div>
              <div>
                <span className="font-semibold">Model:</span>{" "}
                {selectedCar.raw?.carModel}
              </div>
              <div>
                <span className="font-semibold">Seats:</span>{" "}
                {selectedCar.raw?.carSeats}
              </div>
              <div>
                <span className="font-semibold">Oil:</span>{" "}
                {selectedCar.raw?.carOilType}
              </div>
              <div>
                <span className="font-semibold">Engine:</span>{" "}
                {selectedCar.raw?.carEngineType}
              </div>
              <div>
                <span className="font-semibold">Transmission:</span>{" "}
                {selectedCar.raw?.carTransmission}
              </div>
              <div>
                <span className="font-semibold">Power:</span>{" "}
                {selectedCar.raw?.carPower}
              </div>
              <div>
                <span className="font-semibold">Drivetrain:</span>{" "}
                {selectedCar.raw?.carDrivetrain}
              </div>
              <div>
                <span className="font-semibold">Mileage:</span>{" "}
                {selectedCar.raw?.carMileage}
              </div>
              <div>
                <span className="font-semibold">Capacity:</span>{" "}
                {selectedCar.raw?.carCapacity}
              </div>
              <div>
                <span className="font-semibold">Color:</span>{" "}
                {selectedCar.raw?.carColor}
              </div>
              <div>
                <span className="font-semibold">Fuel:</span>{" "}
                {selectedCar.raw?.fuelType}
              </div>
              <div>
                <span className="font-semibold">Gear:</span>{" "}
                {selectedCar.raw?.gearType}
              </div>
              <div>
                <span className="font-semibold">Rating:</span>{" "}
                {selectedCar.raw?.carRating ?? selectedCar.rating ?? "N/A"}
              </div>
              <div>
                <span className="font-semibold">Price/Day:</span> $
                {selectedCar.raw?.carPriceDay ?? selectedCar.pricePerDay}
              </div>
              <div>
                <span className="font-semibold">Category:</span>{" "}
                {selectedCar.raw?.category}
              </div>
              <div>
                <span className="font-semibold">Discount:</span>{" "}
                {selectedCar.raw?.discount}
              </div>
              <div>
              ...
              <div>
                ...
                {selectedCar.raw?.carReviewCount}
              </div>
              <div>
                <span className="font-semibold">Status:</span>{" "}
                {selectedCar.raw?.isBooked ?? selectedCar.status}
              </div>
              <div>
                <span className="font-semibold">Address:</span>{" "}
                {selectedCar.raw?.carAddress}
              </div>
              <div>
                <span className="font-semibold">Postal Code:</span>{" "}
                {selectedCar.raw?.carPostalCode}
              </div>
              <div>
                <span className="font-semibold">District:</span>{" "}
                {selectedCar.raw?.carDistrict}
              </div>
              <div>
                <span className="font-semibold">City:</span>{" "}
                {selectedCar.raw?.carCity}
              </div>
              <div>
                <span className="font-semibold">Country:</span>{" "}
                {selectedCar.raw?.carCountry}
              </div>
            </div>
          </div>
        )}
